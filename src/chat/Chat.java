	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (args[0].equalsIgnoreCase("join")) {
			...
			if (!p.getConfig().contains("chats." + args[1])) {
				player.sendMessage("§4this chat doesn't exist, create one using §c/chat add <chatname>");
				return false;
			}
				...
				if (list.contains(player.getName())) {
					...
					for (String ss : list) {
						if (Bukkit.getPlayerExact(ss) == null) {
							...
							players.sendMessage("§3chat " + s + ": §9" + player.getName() + "§r left this chat");
						}
					}
				}
			}
			...
			for (String ss : list) {
				if (Bukkit.getPlayerExact(ss) == null) {
					...
					Player players = Bukkit.getPlayerExact(ss);
					players.sendMessage("§3chat " + args[1] + ": §9" + player.getName() + "§r joined this chat");
				}
			}
			...
			player.sendMessage("§4you aren't in a chat, use §c/chat join <chatname> §4to join a chat");
						...
						for (OfflinePlayer ss : players) {
							if (ss.getName().equals(args[2])) {
								...
								if (list.contains(black.getName())) {
									...
									player.sendMessage("§3successfully added §9" + black.getName());
						...
						for (OfflinePlayer ss : players) {
							if (ss.getName().equals(args[2])) {
								...
								if (!list.contains(black.getName())) {
									...
									player.sendMessage("§3successfully removed §9" + black.getName());
									return false;
								}
							} else {
								online = true;
			...
			p.getConfig().set("chats." + args[1] + ".players", list);
			p.saveConfig();
			player.sendMessage("§3successfully added the chat §9" + args[1]);
					...
					if (p.getConfig().getString("chats." + s + ".owner").equalsIgnoreCase(player.getName())) {
						...
						for (String ss : list) {
							if (Bukkit.getPlayerExact(ss) == null) {
								...
								Player players = Bukkit.getPlayerExact(ss);
								players.sendMessage("§3chat " + s + ":§c this chat is removed by the owner");
							}
						}

						p.getConfig().set("chats." + s, null);
						p.saveConfig();
						player.sendMessage("§3successfully removed the chat §9" + s);
						return false;
					}
				}
				...
				if (player.isOp()) {
					if (p.getConfig().contains("chats." + args[1])) {
						if (!p.getConfig().getString("chats." + args[1] + ".owner").equals(player.getName())) {
							player.sendMessage("§3successfully removed from an OP position");
							if (Bukkit.getServer()
							...
							for (String ss : list) {
								if (Bukkit.getPlayerExact(ss) == null) {
									...
									Player players = Bukkit.getPlayerExact(ss);
									players.sendMessage("§3chat " + args[1] + ":§c this chat is removed by an OP");
								}
							}
